...
const Profile = () => {
  ...
  const { email, profileName, primaryName, profilePic, primaryPic, address, phn } = userInfo;
  ...
  const updateImage = (data) => {
    ...
    fetch("https://api.imgbb.com/1/upload?key=778aabdeab4b1469f4ccd5b8085229fb", {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
  ...
  return (
    <div className="nested-route-wrapper mb-10">
      <h1 className="text-center text-4xl text-primary my-7 ">{!isEditing ? "My Profile" : "Edit Profile"}</h1>
      {!isEditing && (
        <div className="profile-container">
          <div>
            <div className="profile-pic-wrapper">
              <img className="profile-pic" src={profilePic ? profilePic : primaryPic || avatar} alt="profile-pic" />
            </div>
            <h1 className="text-4xl text-center">{profileName ? profileName : primaryName}</h1>
          </div>
          <ul className="text-primary profile-item-list">
            ...
            <li className="profile-item">
              ...
              <span className="profile-item-name">{phn ? phn : "Not available"}</span>
            </li>
            <li className="profile-item">
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <span className="profile-item-name">{address ? address : "Not available"}</span>
            </li>
          </ul>
            ...
            <div className="form-control">
              <label className="label">
                <span className="label-text">
                  Your Full Address (Please add all details like division, district, locality, road no, house no, and other additional information so
