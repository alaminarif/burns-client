...
const UpdateProfile = () => {
  ...
  const onSubmit = (data) => {
    console.log(data);
    const url = `https://shielded-falls-95338.herokuapp.com/myprofile/${user?.email}}`;
    fetch(url, {
      method: "PUT",
      headers: {
  ...
  return (
    <div className="grid grid-cols-1 justify-items-center py-8">
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 justify-items-center gap-6 w-96 bg-base-100 shadow-xl px-8">
        <input className="input input-bordered w-full " value={user?.email} placeholder="email" type="email" {...register("email")} />
